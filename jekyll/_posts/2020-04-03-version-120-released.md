...
`collect` 是一个用以解析序列/集合/表解析的宏。
它替代了 `lc` (list comprehension，列表解析), 而且比 `lc` 更人性化和强大。
使用 `collect` 没有开销，比如说：你可以把 `newSeqOfCap` 和它配合使用。
...
### Dup
`dup` 是一个宏，它让原地函数（in-place functions，译者注：引用传递参数的函数）
可以在不影响和修改函数输入的情况下返回结果。

我们是这么考虑的，Nim的未来版本将只提供函数的原地版本
（即不再提供 `sort` 和 `sorted`、`rotateLeft` 和 `rotatedLeft` 等），
他们可以被 `dup` 转换为返回拷贝的函数。
这些版本可以通过 `dup` 转换成返回副本的函数。
...
‎`capture` 是一个宏，在循环中创建闭包时非常有用，
以捕获一些局部循环变量（按其当前迭代值）。
您可能希望将其用于事件处理和 `{int: callBack}` 查找表。‎
